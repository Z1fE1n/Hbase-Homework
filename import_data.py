...
class HBaseImporter:
    ...
    def connect(self):
        ...
        # 尝试不同的配置组合
        configs = [
            {'transport': 'buffered', 'protocol': 'binary', 'timeout': 60000},
            {'transport': 'framed', 'protocol': 'binary', 'timeout': 60000},
            {'transport': 'buffered', 'protocol': 'compact', 'timeout': 60000},
        ]
        ...
        with open(csv_path, 'r', encoding='utf-8') as f:
            ...
            with tqdm(total=total_movies, desc="导入电影", unit="部", 
                ...
                for row in reader:
                    ...
                    # 构建数据
                    data = {
                        b'info:title': row['title'].encode('utf-8'),
                        b'info:genres': row['genres'].encode('utf-8'),
                    }
                    
                    # 添加评分统计
                    if movie_id in rating_stats:
                        stats = rating_stats[movie_id]
                        data[b'info:avg_rating'] = f"{stats['avg']:.2f}".encode('utf-8')
                        data[b'info:rating_count'] = str(stats['count']).encode('utf-8')
                    else:
